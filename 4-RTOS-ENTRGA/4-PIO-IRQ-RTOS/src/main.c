...
void but1_callback(void){
	...
	uint32_t delat_change = 100;
	xQueueSendFromISR(xQueueBut_fila,(void *)&delat_change, &xHigherPriorityTaskWoken);
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(xQueueBut_fila, &msg, (TickType_t) 0)) {
      /* atualiza frequencia */
      delayTicks -= msg;
...
int main(void) {
  ...
  xQueueBut_fila = xQueueCreate(32, sizeof(uint32_t));
  if (xQueueBut_fila == NULL)
