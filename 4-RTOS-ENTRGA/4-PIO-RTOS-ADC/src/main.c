...
#define TASK_PROC (1024 * 10 / sizeof(portSTACK_TYPE))
...
static void task_adc(void *pvParameters) {
  ...
  adcData average;
  while (1) {
    if (xQueueReceive(xQueueProc, &(average), 1000)) {
      printf("ADC media: %d \n", average);
    } else {
      //printf("nao chegaram novos dados");
    }
  }
}
...
int main(void) {
  ...
  if (xTaskCreate(task_proc, "PROC", TASK_PROC, NULL,
